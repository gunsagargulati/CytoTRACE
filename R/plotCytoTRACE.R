...
#' The current implementation uses <i>t</i>-SNE for dimensional reduction but users can also input their own embeddings.
...
#' @return a tab-delimited text file saved to disk containing a table of CytoTRACE values, <i>t</i>SNE embeddings, and, if provided, phenotype labels and gene expression values.
#'
...
#' #Run plotCytoTRACE
#' \dontrun{
#' results <- plotCytoTRACE(results, gene = "Kit")
#' }
#' @export
...
if(!is.null(phenotype)){
  ...
  if (length(intersect(colnames(mat), names(pheno))) != length(pheno)) {
    stop("The names of the phenotype labels do no match the names in the CytoTRACE object.", type="error")
  }
  if (max(nchar(pheno))>25) {
    warning("Phenotype labels exceed 25 characters. This may result in overcrowded text in some of the visualizations.", type="warning")
  }
